var web = function(path, params, req, resp) {
	/**
	...
	this.post = function() {
		...
		var value = '';
		var posts = {};
		for(var item in req.post) {
			key = htmlSpecialChars(item);
			if(isString(req.post[item])){
				value = htmlSpecialChars(req.post[item]);
				posts[key] = value;
			}else{
				posts[key] = req.post[item];
			}
		}
	...
	/**
	 * Return the params from uri without param m, c & a.
	 * @return {Object}
	 */
	this.get = function() {
		var key = '';
		...
		for(var item in params) {
			key = htmlSpecialChars(item);
			value = htmlSpecialChars(params[item]);
			gets[key] = value;
		}
