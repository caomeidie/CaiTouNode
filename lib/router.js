	...
	try{
		if(path.length == 3){
			...
			if(FS.existsSync(fs_controller_path)){
				...
				// var web_utility = require('./web_utility');//registe the web utility function
				// var web_utility_obj = new web_utility(req, resp);
				// for(i in web_utility_obj){
				// 	eval(i+'='+web_utility_obj[i]);
				// }
				var file = require(controller_path);
				var file_obj = new file(req, resp, temp);
				var action = path[2]+config.action;
				var html = file_obj[action]();
				// resp.setHeader('Set-Cookie',req.headers.cookie);
...
function cookie(cookie_name, cookie_val){
