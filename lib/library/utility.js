			...
			if(cookie_name instanceof Array){
				...
				for(var item in cookie_name) {
					if(!cookie_obj[item]){
						...
						cookie_obj[item] = cookie_name[item];
					}
				}
				cookie_obj = Object.assign(cookie_obj, obj);
			}else{
				return cookie_obj[cookie_name] || '';
			}
		}else{
			if(!cookie_obj[cookie_name]){
				var obj = {};
				obj[cookie_name] = cookie_val;
				cookie_obj = Object.assign(cookie_obj, obj);
			}else{
				cookie_obj[cookie_name] = cookie_val;
			}
		}
	}
